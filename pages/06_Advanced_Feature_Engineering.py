...
try:
    ...
    PRIMARY_COLOR, SECONDARY_COLOR, SUCCESS_COLOR, WARNING_COLOR = "#2563eb", "#8b5cf6", "#10b981", "#f59e0b"
    DANGER_COLOR, TEXT_COLOR, SUBTLE_TEXT = "#ef4444", "#0f172a", "#64748b"
    def apply_css():
        st.markdown("""
        <style>
        .hero{border-radius:20px;padding:22px;background:
        linear-gradient(135deg,rgba(37,99,235,.10),rgba(139,92,246,.08),rgba(16,185,129,.10));
        border:1px solid rgba(37,99,235,.2);margin-bottom:14px}
        .hero h2{margin:0;font-size:1.6rem;font-weight:800}
        .muted{color:#64748b}
        .chip{display:inline-flex;gap:.5rem;align-items:center;border:1px solid #e5e7eb;
              padding:.35rem .6rem;border-radius:999px;margin-right:.35rem}
        </style>
...
st.set_page_config(page_title="Advanced Feature Engineering", page_icon="🧪", layout="wide")
...
def page_feature_engineering():
    ...
    <div class='hero'>
      ...
      <div class='muted'>Leakage-safe, deduplicated transformations. Choose to skip or scale.</div>
    </div>
    """, unsafe_allow_html=True)
...
def main():
    apply_css()
    init_state()
    header("Advanced Feature Engineering", "Flexible, deduplicated feature creation.", icon="🧪")
