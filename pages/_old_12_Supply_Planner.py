...
if "supply_api_provider" not in st.session_state:
    st.session_state.supply_api_provider = "anthropic"  # Default to Anthropic
    ...
    if st.session_state.inv_optimization_done and st.session_state.inv_optimized_results:
        ...
        if st.session_state.supply_insight_mode == "llm":
            ...
            with st.expander("🔑 LLM API Configuration", expanded=not has_api_key):
                ...
                with provider_col1:
                    if st.button(
                        "🟣 Anthropic (Claude)",
                        type="primary" if st.session_state.supply_api_provider == "anthropic" else "secondary",
                        use_container_width=True,
                        key="supply_provider_anthropic"
                    ):
                ...
                with provider_col2:
                    if st.button(
                        "🟢 OpenAI (GPT)",
                        type="primary" if st.session_state.supply_api_provider == "openai" else "secondary",
                        use_container_width=True,
                        key="supply_provider_openai"
                    ):
                        st.session_state.supply_api_provider = "openai"
                        st.rerun()

                st.markdown("---")

                # API Key input
                provider_name = "Anthropic" if st.session_state.supply_api_provider == "anthropic" else "OpenAI"
                model_name = "Claude Sonnet 4" if st.session_state.supply_api_provider == "anthropic" else "GPT-4o-mini"
        ...
        try:
            ...
            engine = SupplyRecommendationEngine(
                ...
                api_provider=st.session_state.supply_api_provider
            )
