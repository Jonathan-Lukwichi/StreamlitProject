...
def create_kpi_card_html(title: str, value, suffix: str = "", color: str = "#FFFFFF"):
    ...
    return f"""
    <div class="hf-section-card" style="border-left: 4px solid {color};">
        <div class="hf-section-body">{title}</div>
        <div class="hf-section-title" style="color: {color};">{display_val}</div>
    </div>
    """
...
st.set_page_config(
    page_title="XGBoost Lab - HealthForecast AI",
    page_icon="🧠",
    layout="wide",
)
...
<style>
/* ========================================
   FLUORESCENT EFFECTS FOR XGBOOST LAB
   ======================================== */
...
st.markdown(
    ...
    <div class='hf-feature-card' style='text-align: center; margin-bottom: 2rem;'>
      ...
      <h1 class='hf-feature-title' style='font-size: 2.5rem; margin-bottom: 1rem;'>XGBoost Lab</h1>
      <p class='hf-feature-description' style='font-size: 1.125rem; max-width: 700px; margin: 0 auto;'>
        A comprehensive playground for training and tuning XGBoost models with advanced hyperparameter optimization
      </p>
    </div>
    """,
    unsafe_allow_html=True,
    ...
    if cfg_global is None:
        ...
        if st.button("🚀 Train Baseline XGBoost", type="primary", use_container_width=True):
            ...
            if 'xgboost_lab_artifacts' in st.session_state:
                ...
                st.markdown('<div class="hf-section-card">', unsafe_allow_html=True)
