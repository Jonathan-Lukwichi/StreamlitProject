...
def page_hyperparameter_tuning():
    ...
    # Check for individual model results (all use lowercase keys now)
                    ...
                    with st.spinner(f"⏳ Running Grid Search... This may take several minutes."):
                        try:
                            ...
                            try:
                                ...
                                if cloud_success:
                                    st.toast(f"☁️ Saved {selected_model_opt} (Grid) to cloud")
                            except Exception:
                                pass
                            # ==================================================
    ...
    if st.button("🚀 Train LSTM-XGBoost Hybrid", type="primary", use_container_width=True):
        with st.spinner("🔄 Training LSTM-XGBoost hybrid model... This may take several minutes."):
            try:
                ...
                # ===== SAVE HYBRID MODEL TO DISK FOR CLOUD SYNC =====
                try:
                    saved_paths = save_hybrid_model_to_disk("lstm_xgb", artifacts, config)
                    if saved_paths:
                        st.success(f"💾 Saved {len(saved_paths)} hybrid model file(s) to `pipeline_artifacts/hybrids/lstm_xgb/`")
                except Exception as save_err:
                    st.warning(f"⚠️ Could not save hybrid model files: {save_err}")
                # ====================================================
                ...
                try:
                    from app_core.data.cloud_model_sync import save_model_to_supabase
                    cloud_success, cloud_msg = save_model_to_supabase("lstm_xgb_results")
                    if cloud_success:
                        st.toast("☁️ Saved LSTM-XGBoost to cloud")
                except Exception:
                    pass  # Cloud save is optional
                # ==================================================
    ...
    if st.button("🚀 Train LSTM-ANN Hybrid", type="primary", use_container_width=True):
        with st.spinner("🔄 Training LSTM-ANN hybrid model... This may take several minutes."):
            try:
                ...
                try:
                    from app_core.data.cloud_model_sync import save_model_to_supabase
                    cloud_success, cloud_msg = save_model_to_supabase("lstm_ann_results")
                    if cloud_success:
                        st.toast("☁️ Saved LSTM-ANN to cloud")
                except Exception:
                    pass  # Cloud save is optional
                # ==================================================
