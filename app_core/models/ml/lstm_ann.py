...
def build_ann_residual_model(config: Dict[str, Any], n_features: int) -> "tf.keras.Model":
    """Builds and compiles a Keras ANN for the residual modeling stage."""
    if not _tf_available:
        raise ImportError("TensorFlow/Keras is required for the ANN model.")

    model = Sequential()
    model.add(Dense(config["layers"][0], activation="relu", input_shape=(n_features,)))
    model.add(BatchNormalization())
    model.add(Dropout(config["dropout"]))
    if len(config["layers"]) > 1:
        model.add(Dense(config["layers"][1], activation="relu"))
    model.add(Dense(1)) # Output layer for regression

    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=config["lr"]), loss="mean_squared_error")
    return model


class LSTMANNHybrid(BaseHybridModel):
    """
    A hybrid model combining an LSTM (Stage 1) and an ANN (Stage 2).
    - The LSTM learns the primary time-series signal.
    - The ANN model learns to predict the residuals (errors) of the LSTM.
    - The final forecast is the sum of the LSTM forecast and the ANN residual forecast.
    """

    name = "LSTM-ANN"

    def __init__(self, artifact_dir: str = "pipeline_artifacts/hybrids/lstm_ann"):
    ...
    def fit(self, df: pd.DataFrame, target: str, config: Dict[str, Any]) -> HybridArtifacts:
        ...
        # 3. Stage 2: Train ANN on residuals
        ...
        ann_cfg = config["stage2"]
        stage2_model = build_ann_residual_model(ann_cfg, n_features=X_res_tr.shape[1] - 1)
        
        early_stopping = EarlyStopping(
            monitor='val_loss',
            patience=ann_cfg["patience"],
            restore_best_weights=True
        )
        
        stage2_model.fit(
            X_res_tr.drop(columns="resid"), y_res_tr["resid"],
            validation_data=(X_res_val.drop(columns="resid"), y_res_val["resid"]),
        ...
        s2_path = os.path.join(self.artifact_dir, "stage2_ann.keras")
    ...
    def predict(self, df_future: pd.DataFrame, stage1_model, stage2_model, scaler, config: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        y_hat_stage2 = stage2_model.predict(X_future_scaled.iloc[-len(y_hat_stage1):]).flatten()
...
if __name__ == "__main__":
    print("Running smoke test for LSTMANNHybrid...")
    ...
    test_config = {
        ...
        "stage2": { # ANN config
            ...
            "dropout": 0.2,
            "lr": 0.001,
            "epochs": 10, # Low for a quick test
            "batch_size": 16,
    ...
    try:
        ...
        print(f"  - ANN model saved to: {artifacts.stage2_path}")
