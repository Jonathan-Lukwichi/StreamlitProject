...
class LSTMXGBHybrid(BaseHybridModel):
    """
    A hybrid model combining an LSTM (Stage 1) and an XGBoost model (Stage 2).
    - The LSTM learns the primary time-series signal.
    - The XGBoost model learns to predict the residuals (errors) of the LSTM.
    - The final forecast is the sum of the LSTM forecast and the XGBoost residual forecast.
    """

    name = "LSTM-XGBoost"

    def __init__(self, artifact_dir: str = "pipeline_artifacts/hybrids/lstm_xgb"):
    ...
    def fit(self, df: pd.DataFrame, target: str, config: Dict[str, Any]) -> HybridArtifacts:
        ...
        # 3. Stage 2: Train XGBoost on residuals
        ...
        stage2_model.fit(
            X_res_tr.drop(columns="resid"), y_res_tr["resid"],
            eval_set=[(X_res_val.drop(columns="resid"), y_res_val["resid"])],
            verbose=False,
        )

        # 4. Refit Stage 1 LSTM on all training data for final use in `predict`
        ...
        s2_path = os.path.join(self.artifact_dir, "stage2_xgb.json")
        ...
        stage2_model.save_model(s2_path)
    ...
    def predict(self, df_future: pd.DataFrame, stage1_model, stage2_model, scaler, config: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        Returns:
            A tuple of (final_forecast, stage1_forecast, stage2_forecast).
        """
        # Scale features
        ...
        # Stage 2: XGBoost forecast on residuals
        # Note: XGBoost predicts on the same features the LSTM used
        y_hat_stage2 = stage2_model.predict(X_future_scaled.iloc[-len(y_hat_stage1):])
...
if __name__ == "__main__":
    print("Running smoke test for LSTMXGBHybrid...")

    # 1. Create dummy data
    set_seed(42)
    dates = pd.to_datetime(pd.date_range(start="2023-01-01", periods=200, freq="D"))
    data = np.linspace(0, 100, 200) + np.sin(np.arange(200) / 20) * 20 + np.random.randn(200) * 5
    df = pd.DataFrame({"Date": dates, "value": data})

    # 2. Define a configuration
    # TODO: In a real scenario, these would come from a config file or UI
    test_config = {
        "random_state": 42,
        "features": {
            "lags": [7, 14],
        ...
        "eval": {"tail_size": 0.15}, # Use last 15% for final evaluation
        "oof_splits": 4, # For creating residual features
        "stage1": { # LSTM config
            "lookback": 14,
            "units": 50,
            "dropout": 0.1,
            "epochs": 5, # Low for a quick test
            "batch_size": 16,
            "lr": 0.001,
        },
        "stage2": { # XGBoost config
            "n_estimators": 100,
            "max_depth": 5,
            "learning_rate": 0.1,
    ...
    try:
        ...
        print(f"  - XGBoost model saved to: {artifacts.stage2_path}")
        print(f"  - Final evaluation metrics: {artifacts.metrics}")

        # 4. (Optional) Test predict method
        # In a real case, you'd load models from artifacts.
        # For this test, we can't easily load the Keras model without reloading TF,
        # so we'll just confirm the fit process worked.
